...
if($_GET["id_state"]=="free"){
	//	remove from db
	$update="DELETE FROM ".T_BOOKINGS." WHERE id_item='".$id_item."' AND the_date='".$the_date."' AND PropId = '".$_SESSION['PropId']."' LIMIT 1";
	$new_class="";
	$new_desc	=$lang["available"];
}else{
	
	//	get current state
	$sql="SELECT id_state FROM ".T_BOOKINGS." WHERE id_item='".$id_item."' AND the_date='".$the_date."' AND PropId = '".$_SESSION['PropId']."'";
	...
	if(mysql_num_rows($res)==0){
		...
		echo $update		=	"INSERT INTO ".T_BOOKINGS." SET id_item='".$id_item."',the_date='".$the_date."', id_state='".$new_state."',PropId = '".$_SESSION['PropId']."' ";
	}else{
		...
		if($new_state==""){
			...
			$update="UPDATE ".T_BOOKINGS." SET id_state='".$new_state."' WHERE id_item='".$id_item."' AND the_date='".$the_date."' AND PropId = '".$_SESSION['PropId']."' LIMIT 1";
