...
class MY_Controller extends CI_Controller {  
	...
	function __construct()
    {
		...
		/*if(strpos('http://www.',$this->config->item('base_url')) != '' && strpos('http://www.',$this->input->server('HTTP_REFERER')) == '') {
				header("HTTP/1.1 301 Moved Permanently");
				echo 'asd'.str_replace('http://','http://www.',$this->input->server('HTTP_REFERER')); die;
				header("Location:".str_replace('http://','http://www.',$this->input->server('HTTP_REFERER')));
		}
		
		if(strpos('http://',$this->config->item('base_url')) != '' && strpos('http://',$this->input->server('HTTP_REFERER')) == '') {
				header("HTTP/1.1 301 Moved Permanently");
				echo 'asd'.str_replace('http://','http://www.',$this->input->server('HTTP_REFERER')); die;
				header("Location:".str_replace('http://www.','http://',$this->input->server('HTTP_REFERER')));
		}
		die;
		...
		if($this->input->server('HTTPS') != 'on') {
			header("HTTP/1.1 301 Moved Permanently");
			header("Location:".$this->config->item('base_url').$this->input->server('REQUEST_URI'));
		}		
		*/
		/*
		 * Connecting Database
		 ...
		 * Loading Footer Cms
		 */
		$this->data['cmsList'] = $this->db->query('select * from '.CMS.' where `status`="Publish" and `hidden_page`="No"');
		
		//echo '<pre>';print_r($this->data['cmsList']->result_array());die;
		/*
		...
		$this->data['activeLgs'] = serialize($_SESSION['activeLgs']);
...
*/		
		...
		if($this->uri->segment('1') != 'admin')
		{
			...
			$filePath = PPPATH."language/".$selectedLanguage."/".$selectedLanguage."_lang.php";
			if($selectedLanguage != '')
			{
