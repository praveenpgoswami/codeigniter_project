...
{
	if($("#wishlist_count").val() == 0)
	{
		$("#wishlist_warn").html("Atleast create or select one category.");
		return false;
	}
	else return true;
}
		...
		<span class="wishgarea">0 Wish list</span>
				...
				<fieldset class="list-categories">
					...
					if(count($WishListCat->result()) > 0){
						...
						?>
<li><label><input type="checkbox" onclick="change_count();" name="wishlist_cat[]" value="<?php echo $wishlist->id; ?>" <?php if(in_array($productList->row()->id,$WishRentalsArr)){ ?>checked="checked" <?php } ?> /><?php echo $wishlist->name; ?></label></li>
<?php } 
 } ?>

</ul></div></fieldset>
					<fieldset class="new-list">
						<input type="hidden" id="wishlist_count" value="0" />
						...
						<p id="wishlist_warn" style="color:#FF0000; padding:7px;"></p>
					</fieldset>
		...
		<div class="btn-area">
				<input class="btn-add-to-list btn-done" onclick="return check_count();" type="submit" name="submit" value="<?php if($this->lang->line('header_done') != '') { echo stripslashes($this->lang->line('header_done')); } else echo "Done"; ?>"/>
