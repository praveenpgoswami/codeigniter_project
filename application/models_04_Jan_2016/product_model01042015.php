...
public function get_allthe_details($status,$city,$checkin,$checkout,$id){
	...
	public function view_product_details_distance_list($lat,$lon,$condition = ''){
		...
		$select_qry = "select p.id, p.room_type, p.product_title, c.name as city_name, pa.latitude, pa.longitude, u.firstname, u.lastname, u.image as user_image, u.id as userId, ph.product_image as PImg, p.price
        FROM ".PRODUCT." p 
		LEFT JOIN ".PRODUCT_ADDRESS." pa on pa.product_id=p.id
	...
	function view_product1($product_id='')
	{
		...
		pa.country,pa.state,pa.city,pa.post_code,pa.address,pa.apt,pa.latitude,pa.longitude,pa.apt,p.datefrom as DATEFROM,p.dateto as DateTo,
		...
		pb.datefrom,pb.dateto,pb.expiredate,fb.name as pakageName,city.name as cityname,country_list.name as CountryName,states_list.name as StateName
		');
		$this->db->from(PRODUCT.' as p');
		$this->db->join(PRODUCT_ADDRESS.' as pa',"pa.product_id=p.id","LEFT");
		$this->db->join(CITY.' as city',"pa.city=city.id","LEFT");
		$this->db->join(COUNTRY_LIST.' as country_list',"country_list.id=pa.country","LEFT");
		$this->db->join(STATE_TAX.' as states_list',"states_list.id=pa.state","LEFT");
		...
		$this->db->join(FANCYYBOX.' as fb',"fb.id=p.member_pakage","LEFT");
...
// 		echo($Cont2);die;
	...
	function get_contactAll_details(){
	...
	function get_contactAllSeller_details(){
	...
	public function view_product_details_sitemapview($condition = ''){
		$select_qry = "select p.*,
						...
						pa.latitude,pa.longitude,pa.address,pa.post_code,pa.city as CityId,
						c.name as city_name,
						s.name as statename,s.meta_title as statemtitle,s.meta_keyword as statemkey,s.meta_description as statemdesc,s.seourl as stateurl,
	...
	//u.firstname,u.lastname,u.address as UserAddress,u.city as UserCity,u.state as UserState,u.country as UserCountry,u.postal_code as UserPostCode,
		$select_qry = "select p.*,
						...
						c.name as city_name,
						rq.checkin,rq.checkout,rq.NoofGuest,rq.numofdates,rq.serviceFee,rq.totalAmt,rq.Bookingno as Bookingno,
						s.name as statename,s.meta_title as statemtitle,s.meta_keyword as statemkey,s.meta_description as statemdesc,s.seourl as stateurl,
						pp.product_image from ".PRODUCT." p 
		LEFT JOIN ".PRODUCT_ADDRESS." pa on pa.product_id=p.id
		LEFT JOIN ".CITY." c on c.id=pa.city
	...
	public function view_user_details_booking($condition = ''){
		$select_qry = "select u.id as userid,u.user_name,u.email, u.phone_no, rq.NoofGuest,u.feature_product,u.image as userphoto,u.firstname,u.lastname,u.address as UserAddress,u.city as UserCity,u.state as UserState,u.country as UserCountry,u.postal_code as UserPostCode,rq.serviceFee,rq.totalAmt
						 from ".PRODUCT." p 
		LEFT JOIN ".RENTALENQUIRY." rq on rq.prd_id=p.id
		LEFT JOIN ".PRODUCT_BOOKING." b on b.product_id=p.id
	...
	{
		$this->db->select('p.*,
		pa.country,pa.state,pa.city,pa.post_code,pa.address,pa.latitude,pa.longitude,
		pf.feature,pf.google_map,pf.add_feature,pf.rentals_policy,pf.trams_condition,pf.confirm_email,pf.order_email,pf.invoice_template,
		pb.datefrom,pb.dateto,pb.expiredate,u.firstname,u.created as user_created,u.response_rate,u.description as description1,u.phone_no,u.group,u.s_phone_no,u.about,u.email as RenterEmail,u.image as thumbnail,u.about,
		...
		$this->db->join(PRODUCT_FEATURES.' as pf',"pf.product_id=p.id","LEFT");
		$this->db->join(PRODUCT_BOOKING.' as pb',"pb.product_id=p.id","LEFT");
		$this->db->join(LOCATIONS.' as cd',"cd.id=pa.country","LEFT");
	...
	public function get_trip_review($product_id,$reviewer_id='')
	{
	...
	public function get_product_details_wishlist($condition = ''){
		$this->db->select('c.name,p.product_name,pp.product_image,p.id,p.product_title,cd.name as Country_name,st.name as State_name,c.name as CityName');
		$this->db->from(PRODUCT.' as p');
		$this->db->join(PRODUCT_ADDRESS.' as pa',"pa.product_id=p.id","LEFT");
		$this->db->join(PRODUCT_PHOTOS.' as pp',"pp.product_id=p.id","LEFT");
		$this->db->join(LOCATIONS.' as cd',"cd.id=pa.country","LEFT");
	...
	public function get_review_rating($limit='',$limitstart)
	{
		//echo($limit);
...
return $query = $this->db->get();
	...
	public function get_product_details_wishlist_one_category($condition = ''){
		$this->db->select('c.name,p.product_name,p.product_title,p.room_type,p.accommodates,p.bedrooms,p.bathrooms,p.price,n.id as nid,n.notes,p.id,pa.address,pa.post_code');
		$this->db->from(PRODUCT.' as p');
		$this->db->join(PRODUCT_ADDRESS.' as pa',"pa.product_id=p.id","LEFT");
		$this->db->join(PRODUCT_PHOTOS.' as pp',"pp.product_id=p.id","LEFT");
		$this->db->join(CITY.' as c',"c.id=pa.city","LEFT"); 
	...
	public function get_PriceMaxMin($eqid){
	
		$this->db->select('MAX(p.price) as MaxPrice,MIN(p.price) as MinPrice');
		$this->db->from(PRODUCT.' as p');
		$this->db->join(PRODUCT_ADDRESS.' as a',"p.id=a.product_id");
		$this->db->where('p.status','Publish');
		$this->db->where('a.city',$eqid);
		return $query = $this->db->get();
