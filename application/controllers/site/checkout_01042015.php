...
class Checkout extends MY_Controller { 
	...
	public function PaymentCredit()
		{ 	
			...
			$excludeArr = array('firstname','lastname','address','suite','city','state','postal_code','creditvalue','shipping_id','email','total_price','CreditSubmit','creditCardIdentifier','creditvalue','booking_rental_id');
		...
		//echo "hello".$this->input->post('creditvalue'); die;
			if($this->input->post('creditvalue')=='authorize') 
				...
				//echo "<script>alert();</script>";die;
					...
					//echo "test1";print_r($Auth_Setting_Details);die;
					error_reporting(-1);
		...
		//echo '<pre>'; print_r($response); die;
					if($response->approved == '')
						{
							...
							$seller = $this->checkout_model->get_all_details(USERS,array('id' => $product->row()->user_id));
								...
								//$proid=$this->input->post('booking_rental_id');
								//DealCodeNumber
								//$lastFeatureInsertId = $this->session->userdata('randomNo');
								...
								'shipping_status' => 'Pending',
								'total'  => $totalAmount,
								'EnquiryId'=>$this->session->userdata('EnquiryId'),
								'inserttime' => NOW());
								$this->checkout_model->simple_insert(PAYMENT,$paymentArr);
								$insertIds[]=$this->db->insert_id();
								$paymtdata = array(
			...
			else 
				{
					...
					$CCDetails = array('creditcardtype' => 'visa', //$this->input->post('cardType'), 						// Required. Type of credit card.  Visa, MasterCard, Discover, Amex, Maestro, Solo.  If Maestro or Solo, the currency code must be GBP.  In addition, either start date or issue number must be specified.
									'acct' => '4111111111111111', //$this->input->post('cardNumber'), 								// Required.  Credit card number.  No spaces or punctuation.  
									'expdate' => '012018', //$this->input->post('CCExpDay').$this->input->post('CCExpMnth'),// Required.  Credit card expiration date.  Format is MMYYYY
									'cvv2' => '1234', //$this->input->post('creditCardIdentifier'), 						// Requirements determined by your PayPal account settings.  Security digits for credit card.
									'startdate' => '', 							// Month and year that Maestro or Solo card was issued.  MMYYYY
									'issuenumber' => ''							// Issue number of Maestro or Solo card.  Two numeric digits max.
									);
					
					$PayerInfo = array('email' => 'rams@gmail.com',//$this->input->post('email'), 	// Email address of payer.
					...
					$PayerName = array('salutation' => 'Mr.', 						// Payer's salutation.  20 char max.
									'firstname' => 'Aravinth', //$this->input->post('full_name'),	// Payer's first name.  25 char max.
...
//'x_amount'				=> ,
	//			'x_email'				=> $this->input->post('email'),
					...
					/*$ShippingAddress = array('shiptoname' => $shipValID->row()->full_name,		// Required if shipping is included.  Person's name associated with this address.  32 char max.
					...
					$PaymentDetails = array('amt' => '200', //$this->input->post('total_price'), 							// Required.  Total amount of order, including shipping, handling, and tax.  	
		...
		// collection of all items in $OrderItems.
					...
					$PayPalRequestData = array('DPFields' => $DPFields, 
											'CCDetails' => $CCDetails, 
											'PayerInfo' => $PayerInfo, 
											'PayerName' => $PayerName, 
